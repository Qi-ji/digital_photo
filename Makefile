...
# c编译器在编译时的参数设置
CFLAGS := -Wall -O2 -g -finput-charset=GBK -fexec-charset=GBK
# 添加头文件路径，不添加的话include目录下的头文件编译时找不到
#CFLAGS += -I $(shell pwd)/include 
CFLAGS += -I/opt/libdecode/include
CFLAGS += -I/opt/libdecode/include/freetype2/

# 链接器的链接参数设置
#LDFLAGS := -ljpeg -lz -lpng -L/opt/libdecode/lib
LDFLAGS := -lfreetype -lm -L/opt/libdecode/lib
...
cp:
	cp ../text_showby_freetype/ /root/rootfs/rootfs -rf
